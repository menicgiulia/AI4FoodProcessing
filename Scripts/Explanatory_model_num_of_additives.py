...
# -------------------------------
# We only use 'additives_n' as a feature (raw count, no log-transform)
feature = 'additives_n'
print(f"Loading OFF data from {off_file}...")
df_off = pd.read_csv(off_file, sep='\t')
# Drop rows missing 'additives_n' or 'nova_group'
df_off = df_off.dropna(subset=[feature, 'nova_group'])

# Prepare feature matrix X and label y
X = df_off[[feature]].values
