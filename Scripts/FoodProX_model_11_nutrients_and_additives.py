...
nut_off = [
 ...
 'cholesterol_100g','saturated-fat_100g','trans-fat_100g','additives_n'
]

print(f"Loading OFF data from {off_file}...")
df_off = pd.read_csv(off_file, sep='\t')
# Drop rows with missing nutrient or nova_group values
df_off = df_off.dropna(subset=nut_off + ['nova_group'])

# Log-transform only the 11 nutrient columns; replace -inf with -20
nutrients = nut_off[:-1]  
